...
export function SpendingChart({ summary }: SpendingChartProps) {
  ...
  return (
    <section className="receipt-tape p-6 pt-9">
      ...
      {total === 0 ? (
        <div className="mt-8 rounded-[2rem] border border-dashed border-primary/30 bg-background/60 p-6">
        ...
        <div className="mt-7 grid gap-6 sm:grid-cols-[13rem_1fr] sm:items-center">
          ...
          <div className="space-y-3">
            {slices.map((slice) => (
              <div
                key={slice.item.category_id ?? slice.item.category_name}
                className="flex items-center justify-between gap-3 rounded-2xl bg-background/65 px-4 py-3"
              >
