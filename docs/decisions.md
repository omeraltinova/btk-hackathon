...
### 2026-05-18 — Day 8 — Voice chat keeps Gemini Live as transport, not the coach brain

**Context:** The user wanted a second control beside the microphone: direct Gemini mode should use a realtime voice model, while OpenRouter mode should compose STT → LLM → TTS because OpenRouter has no realtime voice API surface.
**Decision/learning:** Bumped `master_plan.md` to v0.38 and added authenticated `/api/voice/session`. In direct Gemini mode it mints a short-lived ephemeral token for `gemini-3.1-flash-live-preview`; in OpenRouter mode it returns `cascade` mode without exposing provider secrets. The new frontend headphones button starts Gemini Live when available, but Live is only the realtime transport layer: every substantive voice turn is delegated through one function bridge back into the existing `/api/chat/stream` path, so scoped tools, approval cards, and persisted conversation history stay identical to typed chat. OpenRouter uses the same button as a one-turn `STT → existing chat/agent → TTS` flow, and Gemini Live setup/runtime failures fall back to that same non-realtime chain instead of becoming a dead-end UX.
**Why it matters:** Do not duplicate the finance tool surface inside Gemini Live or bypass the existing agent runner for voice-only answers; that would fork authorization, approval, and product behavior. Future voice work should treat Live as an I/O shell around the current coach, not as a separate product brain.
**Verification:** Added backend voice-session tests and ran `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy app`, and `uv run pytest -q` (167 tests passed). Frontend direct checks passed with local binaries: `tsc --noEmit`, `eslint .`, `prettier --check .`, and `next build`. Rebuilt the stack with `docker compose up -d --build`, ran `docker compose exec -T backend uv run alembic upgrade head`, confirmed healthy services with `docker compose ps`, verified unauthenticated `/api/voice/session` returns `401`, and authenticated OpenRouter mode returns `{"provider":"openrouter","mode":"cascade",...}`. The currently configured live stack is in OpenRouter mode, so direct Gemini Live behavior is covered by unit tests and code review rather than a live token smoke in this pass.
**Reference:** `docs/master_plan.md` v0.38, `backend/app/services/voice.py`, `backend/app/routers/voice.py`, `backend/tests/test_voice.py`, `frontend/lib/live-voice.ts`, `frontend/components/ChatStream.tsx`, `frontend/lib/types.ts`.

### 2026-05-18 — Day 8 — OpenRouter voice chat is a persistent listening loop

**Context:** Manual OpenRouter-mode testing showed the headphones button could collect speech and produce a chat answer, but it felt like a one-shot microphone: the answer was not reliably spoken aloud and the user had to press the button again for the next turn.
**Decision/learning:** Bumped `master_plan.md` to v0.39 and changed the OpenRouter cascade path into a persistent voice-session panel. The panel stays open until the user closes it, uses Web Audio RMS/silence detection to start/stop `MediaRecorder` automatically, sends each finished utterance through the existing scoped `/api/stt` + `/api/chat/stream` path, awaits `/api/tts` playback, then returns to listening. Provider TTS playback now resolves when audio ends so the mic does not immediately capture the assistant's own voice; browser speech fallback is also awaited when provider TTS fails.
**Why it matters:** Keep the one-shot microphone button separate from the headphones voice-session mode. Future changes to cascade voice should preserve the turn order `listen → transcribe → stream coach → speak → listen` and should not create a second finance-answering path outside the current agent runner.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, and `corepack pnpm build` passed.
**Reference:** `docs/master_plan.md` v0.39, `frontend/components/ChatStream.tsx`, `frontend/lib/tts.ts`.

### 2026-05-18 — Day 8 — Voice overlay owns approvals and visual previews

**Context:** Voice mode felt too disconnected when a tool needed approval or produced a concept image, and empty/no-speech notifications were noisy during hands-free use.
**Decision/learning:** The voice session overlay now renders pending approval cards directly in the centered modal and resumes the cascade loop after approve/reject. Image events generated during voice mode are shown as a temporary in-modal preview before fading back to the voice status copy. No-speech failures in the voice-session loop no longer raise corner toasts; the loop silently returns to listening.
**Why it matters:** Voice mode should keep the user's attention in one surface. Do not send approval decisions through natural-language parsing; keep using the existing `approval_id`/`approval_decision` path from the overlay controls.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, and `corepack pnpm build` passed.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Voice approval pauses listening until the approved tool completes

**Context:** After approving a tool in voice mode, the cascade loop could return to microphone listening too early, making the flow feel like it expected another spoken input instead of continuing the approved operation.
**Decision/learning:** Centralized cascade listening into `pauseCascadeListening()` and `resumeCascadeListening()`. `approval_required` now pauses any active recognition/recorder, approval clicks keep the overlay in `thinking/speaking` until the backend approval stream and TTS finish, and listening only resumes if there is no new pending approval and no active stream/transcription.
**Why it matters:** The voice state machine should only have one place that restarts listening. Approval is not a new user voice turn; it is a continuation of the existing tool workflow through the existing `approval_id` contract.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, and `corepack pnpm build` passed.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Duplicate category names must resolve deterministically for goals

**Context:** Approving a voice-created `create_saving_goal` for `Market` produced a frontend `network error`; backend logs showed SQLAlchemy `MultipleResultsFound` from `resolve_goal_category()` because both system and user-owned `Market` categories were visible.
**Decision/learning:** `resolve_goal_category()` no longer uses `scalar_one_or_none()` for name lookup. It collects exact case-insensitive matches and chooses deterministically: active user's category first, then system default, then other visible family categories. This preserves scoped visibility while avoiding SSE stream crashes.
**Why it matters:** Category names are not globally unique because user shadow categories coexist with system defaults. Any future name-based category resolver should expect duplicates and use the same active-user-first ordering instead of assuming one row.
**Verification:** Added `test_build_saving_goal_creation_prefers_user_category_when_name_duplicates`; `uv run pytest tests/test_agent_tools.py -q`, `uv run ruff check app tests`, `uv run mypy app`, frontend `type-check`, `lint`, `format:check`, and `build` passed.
**Reference:** `backend/app/services/saving_goals.py`, `backend/tests/test_agent_tools.py`.

### 2026-05-18 — Day 8 — Goal category lookup must prefer recent spending before ownership

**Context:** The deterministic duplicate-name fix stopped stream crashes, but approving a `market` saving goal could still choose the wrong same-name category row: the visible spending list showed `Market` totals from one `category_id`, while goal creation selected another `Market` row with no recent transactions and returned “son 30 günde gider bulunamadı.”
**Decision/learning:** For expense-reduction saving goals, `resolve_goal_category()` now accepts the 30-day spending window and first picks an exact name match that has recent spending for the active user. Only if none has spending does it fall back to ownership ordering. This aligns “hangi kategorilerde harcadım?” with “bu kategori için hedef oluştur.”
**Why it matters:** Name-based category lookup is not enough when shadow/system categories coexist. Any goal or plan resolver that depends on category spend should prefer the category row that actually owns the scoped transactions in the relevant period.
**Verification:** Added `test_build_saving_goal_creation_uses_spent_duplicate_category`; `uv run pytest tests/test_agent_tools.py -q`, `uv run ruff check app tests`, `uv run ruff format --check .`, and `uv run mypy app` passed.
**Reference:** `backend/app/services/saving_goals.py`, `backend/tests/test_agent_tools.py`.

### 2026-05-18 — Day 8 — Backend dev container does not mount tests

**Context:** After rebuilding `backend` and `frontend`, follow-up verification attempted to run `docker compose exec -T backend uv run pytest tests/test_agent_tools.py -q` and `docker compose exec -T backend uv run ruff check app tests`.
**Decision/learning:** The Compose dev backend mounts only `./backend/app`, `./backend/alembic`, and `./backend/alembic.ini` into `/app`; `backend/tests` is not present inside the running container image/volume. Container checks that only need `app` still work (`ruff format --check .`, `mypy app`), but test-suite checks must run on the host from `backend/` unless Compose is changed to mount tests.
**Why it matters:** Do not treat `file or directory not found: tests/test_agent_tools.py` inside the dev container as a product regression. For quick post-rebuild checks, use container health/logs plus host-side `uv run pytest ...` and `uv run ruff check app tests`, or update `docker-compose.yml` deliberately if containerized tests become required.
**Verification:** `docker compose up -d --build backend frontend`, `docker compose exec -T backend uv run alembic upgrade head`, `docker compose ps`, backend `/health`, frontend `/`, backend logs, host `uv run pytest tests/test_agent_tools.py -q`, host `uv run ruff check app tests`, host `uv run ruff format --check .`, host `uv run mypy app` all passed after the category resolver fix.
**Reference:** `docker-compose.yml` backend volumes, `backend/Dockerfile`.

### 2026-05-18 — Day 8 — Saving goal baseline must match spending scope

**Context:** Voice approval still returned “son 30 günde gider bulunamadı” for `Market` even though `get_spending` and the dashboard showed recent Market expenses. Local DB inspection showed a 0-spend user-owned `Market` shadow category next to a system `Market` category with recent family transactions.
**Decision/learning:** This was not a prompt/category-list problem: the approval payload already sent `category: Market`. `resolve_goal_category()` now uses visible-family spend when choosing among duplicate names, and the baseline calculation in `build_saving_goal_draft()` also uses `visible_user_ids(current_user)` so it matches `get_spending` for parent scope. Goal progress rehydrates the goal owner and uses that owner's visible scope so family-scoped goals stay coherent after creation.
**Why it matters:** Any feature that says “son 30 gün harcaman” after a parent-scoped spending summary must compute from the same visible family scope, not just `current_user.id`. Otherwise the UI/agent can correctly identify a category but the mutation path rejects it as empty.
**Verification:** Added `test_build_saving_goal_creation_uses_visible_family_spending`; `uv run pytest tests/test_agent_tools.py -q` passed with 42 tests, plus backend `ruff`, `ruff format --check`, and `mypy`. Local container smoke for both `ayse@demo.cuzdan-kocu.app` and `mehmet@demo.cuzdan-kocu.app` now builds a `Market` saving-goal draft with `2098.00` baseline and `1888.20` target spending.
**Reference:** `backend/app/services/saving_goals.py`, `backend/tests/test_agent_tools.py`.

### 2026-05-18 — Day 8 — Voice session overlay has manual escape controls

**Context:** Hands-free OpenRouter cascade voice could get stuck from the user's perspective when the assistant entered speaking/thinking after silence or long TTS playback; the user had no in-modal way to interrupt the coach or pause the microphone without closing the whole session.
**Decision/learning:** The centered voice overlay now exposes two cascade-only controls: `Mikrofonu kapat/aç` pauses or resumes automatic listening while keeping the session panel open, and `Yanıtı durdur` cancels active provider/browser TTS and aborts an in-flight stream if needed. User-paused microphone state blocks automatic `resumeCascadeListening()` until explicitly reopened, and the audio track is disabled while paused so the analyzer does not keep sampling ambient sound.
**Why it matters:** Voice mode needs a user escape hatch. Future cascade changes should preserve three distinct states: session closed, session open but mic paused by the user, and session open with automatic listen/speak looping. Approval pauses are temporary workflow pauses and must not overwrite the user-paused state.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, and `corepack pnpm build` passed.
**Reference:** `frontend/components/ChatStream.tsx`, `frontend/lib/tts.ts`.

### 2026-05-18 — Day 8 — Voice overlay mirrors live streamed answer text

**Context:** The regular chat message already appended SSE `delta` events, but the centered voice overlay only showed status copy until the full assistant answer finished and TTS started. The live LangGraph path also used `stream_mode="updates"`, which meant real provider tokens were not passed through; the backend emitted word chunks only after each AI message was complete.
**Decision/learning:** `sendMessage()` now accepts an `onDelta` callback so voice-chat turns can mirror partial assistant text inside the overlay while the normal chat bubble continues streaming. `_stream_live_graph()` now requests LangGraph `stream_mode=["messages", "updates"]` and forwards `AIMessageChunk` content as SSE `delta` events immediately; the final update is used for persistence and tool handling without duplicating already streamed text.
**Why it matters:** Chat and voice should both feel like ChatGPT/Gemini-style progressive generation. Keep tool/approval events on `updates`, but pass token chunks from `messages` through as soon as they arrive so OpenRouter/Gemini latency is visible rather than a blank “thinking” state.
**Verification:** Added `test_live_graph_streams_message_chunks_without_duplicate_final_delta`; `uv run pytest tests/test_chat_stream.py -q`, backend `ruff`, backend `ruff format --check`, backend `mypy`, frontend `type-check`, frontend `lint`, frontend `format:check`, and frontend `build` passed.
**Reference:** `backend/app/services/agent_runner.py`, `backend/tests/test_chat_stream.py`, `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Voice thinking state waits for a transcript

**Context:** After the assistant finished speaking, the cascade microphone could briefly detect ambient noise, stop a tiny recording, and switch the overlay to “Ses metne çevrildi, mevcut sohbet koçu yanıtı hazırlıyor” even when the user had not spoken.
**Decision/learning:** The cascade recorder no longer sets `voiceChatStatus="thinking"` when a recording stops. The UI stays in listening/transcribing until `/api/stt` or browser fallback returns a non-empty transcript; empty or very small voice-chat clips are ignored and the loop returns to listening without a corner toast.
**Why it matters:** “Koç cevabı hazırlıyor” must mean a real user utterance was captured. Future voice-state changes should avoid optimistic thinking states before transcript validation, because ambient-noise false positives make the assistant look like it is answering itself.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, and `corepack pnpm format:check` passed.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-19 — Day 8 — Gemini Live tokens must use google-genai SDK

**Context:** In direct Gemini mode, pressing the live voice button fell back to cascade mode because `/api/voice/session` returned `503`. Backend logs showed the hand-written `POST https://generativelanguage.googleapis.com/v1alpha/authTokens` call returning `404`, even though the same API key could list `gemini-3.1-flash-live-preview` and normal Gemini chat worked.
**Decision/learning:** Switched `VoiceSessionService` to the official `google-genai` SDK flow from the `gemini-live-api-examples-main/gemini-live-ephemeral-tokens-websocket` reference: `genai.Client(..., http_options={"api_version": "v1alpha"}).auth_tokens.create(...)`. Added `google-genai` as an explicit backend dependency instead of relying on `langchain-google-genai` transitively, and ignored the local `gemini-live-api-examples-main/` reference folder so it is not committed.
**Why it matters:** Do not reintroduce raw `authTokens` HTTP calls unless Google documents a stable REST contract; the SDK currently maps token creation to the working API surface. Keep API keys server-side and only return the ephemeral token to the browser.
**Verification:** `uv run pytest tests/test_voice.py -q`, `uv run ruff check app tests`, `uv run ruff format --check .`, `uv run python -m mypy app`, `docker compose up -d --build backend frontend`, backend `/health`, `docker compose ps`, and an authenticated `/api/voice/session` smoke passed with `provider=gemini`, `mode=realtime`, `has_ephemeral_token=true`.
**Reference:** `backend/app/services/voice.py`, `backend/tests/test_voice.py`, `backend/pyproject.toml`, `backend/uv.lock`, `.gitignore`.

### 2026-05-19 — Day 8 — Gemini Live browser path uses raw token WebSocket

**Context:** After token creation was fixed, direct Gemini mode still showed “Canlı sesli sohbet kesildi” because the frontend passed the ephemeral token to `@google/genai` as if it were an API key. The Google raw WebSocket example uses the ephemeral token as `access_token` on `BidiGenerateContentConstrained`, then sends an explicit `setup` message.
**Decision/learning:** Replaced the browser-side `@google/genai` `ai.live.connect()` path in `GeminiLiveVoiceSession` with the raw WebSocket ephemeral-token protocol. The setup message now mirrors the reference shape: `models/<GEMINI_LIVE_MODEL>`, `generationConfig.responseModalities=["AUDIO"]`, `speechConfig.voiceConfig.prebuiltVoiceConfig.voiceName`, transcript flags, system instruction, and the existing `continue_coach_turn` function declaration. Microphone capture is pinned to 16 kHz PCM and chunks are sent as `realtimeInput.audio`; tool responses are sent as `toolResponse.functionResponses` so the scoped chat/agent bridge remains unchanged.
**Why it matters:** Ephemeral Live tokens are bearer tokens for the constrained WebSocket URL, not normal browser API keys. Keep OpenRouter/cascade logic separate; only the direct Gemini realtime transport should use this raw WebSocket path.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, `corepack pnpm build`, `docker compose up -d --build frontend`, frontend HTTP 307 smoke, backend `/health`, and `docker compose ps` passed. Manual browser mic/WebSocket behavior still needs one user smoke because local tools cannot drive microphone permission here.
**Reference:** `frontend/lib/live-voice.ts`, `gemini-live-api-examples-main/gemini-live-ephemeral-tokens-websocket/frontend/geminilive.js`.

### 2026-05-18 — Day 8 — Voice controls are inline, not modal

**Context:** The centered blurred voice modal hid the chat timeline, so users could not see graphs, images, or normal approval cards while hands-free mode was active.
**Decision/learning:** Removed the voice modal/blur overlay and replaced it with a compact inline voice strip above the chat input. The strip shows status, partial streamed text, mic pause/resume, interrupt, and close controls. Approval cards and generated images now stay in the normal chat timeline; approval button clicks route through the voice approval continuation when cascade mode is active.
**Why it matters:** Voice mode should augment the chat, not cover it. Future voice UI should keep charts, attachments, and approval cards in the main conversation surface unless there is a clear accessibility reason to isolate them.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, `corepack pnpm build`, `docker compose up -d --build frontend`, frontend 200 smoke, backend health smoke, and `docker compose ps` passed.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Inline voice strip avoids duplicate content and stays open

**Context:** After moving voice controls inline, the strip still duplicated streamed answer text already shown in the chat bubble and showed status chips like `Otomatik gönderim`. Browser speech/no-speech failures could also close cascade mode, breaking the continuous conversation flow.
**Decision/learning:** Removed duplicate streamed answer text and the status chips from the inline voice strip; the strip now only shows status plus controls. In cascade browser fallback, speech-recognition errors restart listening instead of closing the session. If mic startup/recording actually fails, the session stays open in the same user-paused state as `Mikrofonu kapat`, so the user can press `Mikrofonu aç` to continue.
**Why it matters:** The chat timeline is the single source for assistant content, charts, images, and approvals. Voice controls should not duplicate content, and temporary no-speech/browser issues should not fully tear down the hands-free session.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, and `corepack pnpm format:check` passed.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Empty cascade clips must clear the transcribing guard

**Context:** After a period of silence, the OpenRouter cascade voice session could look like the microphone had closed. The root cause was not full session teardown: tiny ambient recordings entered `handleRecordedAudio()`, threw before STT, called `resumeCascadeListening()` while `isTranscribingRef.current` was still true, and the resume guard rejected the restart.
**Decision/learning:** Empty/no-speech cascade clips now clear both React state and the synchronous transcribing ref before scheduling recovery through `recoverCascadeListening()`. VAD also now uses the existing adaptive noise-floor constants and requires multiple speech frames before starting a recorder, so one-off ambient spikes are less likely to create empty clips.
**Why it matters:** Any cascade path that wants to restart listening must clear the synchronous refs used by `resumeCascadeListening()` first. Do not call the resume path while `isTranscribingRef`, `isStreamingRef`, or the user pause reason still blocks it unless that block is intentional.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, `corepack pnpm build`, `docker compose up -d --build frontend`, frontend 200 smoke, backend health smoke, and `docker compose ps` passed. Real microphone behavior still needs manual browser confirmation.
**Reference:** `frontend/components/ChatStream.tsx`.

### 2026-05-18 — Day 8 — Default theme is explicit light

**Context:** The app had a top-bar theme toggle, but `next-themes` still used `defaultTheme="system"`, so first-time visitors on dark OS preferences opened directly in dark mode.
**Decision/learning:** `ThemeProvider` now uses `defaultTheme="light"` with `enableSystem={false}` so the first visit always shows the warm ledger light palette while still preserving manual light/dark selection through the existing toggle. Added `frontend/app/icon.svg` as the App Router favicon, matching the household ledger/wallet visual language.
**Why it matters:** Do not reintroduce system-default theming unless the product direction changes; the intended first impression is the light paper-ledger interface, with dark mode as an explicit user choice.
**Verification:** `corepack pnpm type-check`, `corepack pnpm lint`, `corepack pnpm format:check`, and `corepack pnpm build` passed. The build output includes static `/icon.svg`.
**Reference:** `frontend/components/theme-provider.tsx`, `frontend/app/icon.svg`.

### 2026-05-19 — Day 8 — README became the jury/demo entry point

**Context:** Updated public-facing documentation after the feature set stabilized and the team asked for a detailed project explanation, OS-specific run instructions, screenshot guidance, and explicit Gemini/AI usage notes.
**Decision/learning:** `README.md` now acts as the broad demo/jury entry point: project summary, demo accounts, screenshot plan, feature inventory, AI/Gemini/OpenRouter usage, architecture, OS-specific prerequisites, Docker path, host-dev path, env variables, smoke tests, troubleshooting, and deploy summary. `SETUP.md` remains the operational setup guide, but stale route/migration/router references were refreshed to match the flattened routes and current backend API surface. Added `docs/screenshots/README.md` so future screenshots can be committed with consistent names and a reminder to avoid secrets or personal data in images. `.env.example` and Compose demo env passthrough now include all current demo accounts (Ayşe, Mehmet, Elif, Deniz, Zeynep, Kerem) so the login selector and seed command stay configurable together.
**Why it matters:** Future README changes should stay aligned with actual implemented routes and provider behavior, especially `/transactions` as the receipt/OCR surface and voice mode's split between Gemini realtime and OpenRouter cascade. If screenshots are added, use the documented filenames and scrub sensitive data first.
**Reference:** `README.md`, `SETUP.md`, `.env.example`, `docker-compose.yml`, `docker-compose.prod.yml`, `docs/screenshots/README.md`.

### 2026-05-19 — Day 8 — README2 audit kept only code-backed claims

**Context:** Reviewed a second generated README draft (`README2.md`) for possible wording and feature-detail improvements.
**Decision/learning:** Pulled in only verified content: stronger "AI is embedded across the product" intro, `is_demo=true` demo-account note, dashboard projection/share/benchmark details, onboarding tour, child badges, target milestone messaging, and coach-memory recall wording. Rejected inaccurate claims from the draft, including `/api/voice/live` and `voice_live.py`, `POST /api/reports/monthly`, OCR through `GEMINI_IMAGE_MODEL`/OpenRouter image model, compact density toggle, and unsupported extra insight types.
**Why it matters:** README is a jury-facing source, so do not copy generated docs wholesale. Check every impressive-sounding claim against code before adding it.
**Reference:** `README.md`, `README2.md`, `frontend/components/*`, `backend/app/routers/reports.py`, `frontend/lib/live-voice.ts`.

### 2026-05-19 — Day 8 — README factual audit tightened user-facing claims

**Context:** Performed a top-to-bottom README audit against backend routers/services, frontend routes/components, Docker Compose, `.env.example`, and setup/deploy docs.
**Decision/learning:** Tightened claims that were true in code but misleading in README wording: UI receipt upload is image-only even though backend supports `text/plain` fixtures; family breakdowns live on `/family`, not the dashboard; registration only creates parent/individual while child profiles are parent-created; OpenRouter voice looping is frontend cascade behavior; host-mode backend also needs `MINIO_ENDPOINT=localhost:9000`; `NEXT_PRIVATE_API_URL` is a Compose default, not listed in `.env.example`; MinIO public endpoint does not mean reports are public. Production env notes now include the Postgres/MinIO/demo seed variables required by the deploy runbook.
**Why it matters:** Keep README language user-facing and precise. If a feature is backend-only, test-only, frontend-only, or deploy-default-only, say so explicitly instead of presenting it as a universal product capability.
**Reference:** `README.md`, `backend/app/routers/receipts.py`, `frontend/components/ReceiptUploader.tsx`, `frontend/components/auth/register-form.tsx`, `frontend/lib/live-voice.ts`, `docker-compose.yml`, `docs/deploy.md`.

### 2026-05-19 — Day 8 — README2 product narrative was narrowed before import

**Context:** Final README pass looked for any remaining useful content in `README2.md` after rejecting stale technical claims.
**Decision/learning:** Added only two code-backed, product-level sections to `README.md`: the user problem and a compact differentiation table. Kept the wording provider-neutral where needed, avoided the draft's unsupported Gemini Vision/image-model OCR claim, and described proactivity as rule-based dashboard coach notes rather than invented insight types.
**Why it matters:** Jury-facing copy can be persuasive, but the README should still be auditable against the repo. Future generated documentation should be treated as source material, not as a file to copy wholesale.
**Reference:** `README.md`, `README2.md`, `docs/master_plan.md` §1-8.

### 2026-05-19 — Day 8 — README screenshot section became jury-facing

**Context:** The README still contained editor-facing screenshot guidance such as missing-image notes and suggested filenames, which would be visible to jurors.
**Decision/learning:** Replaced the screenshot guidance table with direct Markdown image references under product-facing headings, removed feature-section source-file lists from the README, and deleted the internal `docs/screenshots/README.md` helper. The screenshot paths remain stable so image files can be added later without changing the README text.
**Why it matters:** The public README should read as final presentation material, not as instructions to the team. Keep internal screenshot hygiene notes outside jury-facing docs if they are reintroduced.
**Reference:** `README.md`.

### 2026-05-19 — Day 8 — Screenshot asset guide moved out of README

**Context:** The README should render screenshots directly for jurors, while the team still needs a clear filename guide for adding those images and optional extra demo assets.
**Decision/learning:** Kept the main README screenshot section as direct Markdown image references and recreated `docs/screenshots/README.md` as an internal asset guide listing required README image names plus optional extra demo screenshots. The guide includes a security checklist for secrets and personal data.
**Why it matters:** Do not put screenshot TODO/instruction text back into the main README. Team-facing image logistics belong in `docs/screenshots/README.md`; juror-facing screenshots belong in `README.md`.
**Reference:** `README.md`, `docs/screenshots/README.md`.

### 2026-05-19 — Day 8 — SETUP host and Docker paths clarified

**Context:** Re-reviewed `SETUP.md` before sharing it with a teammate for local setup.
**Decision/learning:** Clarified that Docker-first migration and worker commands should run inside the backend container, while host-side backend commands need `DATABASE_URL=...localhost...` and `MINIO_ENDPOINT=localhost:9000` overrides. Added all current demo account logins, included demo seed in the smoke test, removed stale Day 1 wording, simplified OCR provider wording, and replaced destructive Windows line-ending recovery commands with non-destructive status/check commands.
**Why it matters:** The default `.env.example` uses Docker service names (`postgres`, `minio`), so host-side `make backend`/`make migrate` can fail unless the user overrides env values or edits `.env`. Keep future setup changes explicit about Docker vs host execution context.
**Reference:** `SETUP.md`, `.env.example`, `Makefile`.

### 2026-05-19 — Day 8 — Login receipt layout widens only at xl

**Context:** Manual visual review showed the login page feeling too tall and nested: the demo-account selector stacked above the email/password form inside a narrow receipt panel, and the two-column hero/form layout could feel squeezed.
**Decision/learning:** The login page now waits until `xl` before showing the desktop hero beside the receipt, uses a wider desktop shell, and places demo accounts beside the credential form inside `LoginForm` on wide screens. The demo selector keeps grouped role context but uses responsive grids/truncation so it shortens without overflowing.
**Why it matters:** Future auth-page polish should preserve the `xl` breakpoint for the split layout; reintroducing a `lg` two-column split or stacking demo accounts above the form will likely bring back the cramped, long receipt problem.
**Verification:** `corepack pnpm format:check`, `corepack pnpm lint`, `corepack pnpm type-check`, and `corepack pnpm build` passed.
**Reference:** `frontend/app/(auth)/login/page.tsx`, `frontend/components/auth/login-form.tsx`.

### 2026-05-19 — Day 8 — Login hero copy must not own desktop width

**Context:** Follow-up screenshot showed the large Turkish headline clipping inside the left ledger card, while the login form looked narrow when demo accounts were not available from the API yet.
**Decision/learning:** Rebalanced the desktop grid toward the auth receipt, reduced the hero headline clamp, added word-break protection, and made `LoginForm` use the wide two-column demo/form grid only when demo accounts are actually loaded. The standalone demo explainer below the form was removed because it became misleading when the selector was absent.
**Why it matters:** Long Turkish words in display type need explicit width and scale limits; do not solve auth-page density by giving the marketing card more text size. Demo-account helper copy should live with the selector, not outside it.
**Verification:** `corepack pnpm format:check`, `corepack pnpm lint`, `corepack pnpm type-check`, and `corepack pnpm build` passed.
**Reference:** `frontend/app/(auth)/login/page.tsx`, `frontend/components/auth/login-form.tsx`.

### 2026-05-19 — Day 8 — Local CORS must include 127.0.0.1

**Context:** After a Docker rebuild, `/api/auth/demo-accounts` returned 200 from `localhost:8000`, but the login page could still show `Demo hesaplar yüklenemedi` when opened from `127.0.0.1:3000` or `0.0.0.0:3000`.
**Decision/learning:** Expanded local `APP_CORS_ORIGINS` defaults to include `http://localhost:3000`, `http://127.0.0.1:3000`, and `http://0.0.0.0:3000`. Also updated the local `.env` copy so the currently running Compose backend picks up the same allowed origins.
**Why it matters:** Browser CORS uses the exact origin shown in the address bar; `localhost` and `127.0.0.1` are different origins. Future local setup docs/config should keep both variants unless the frontend is made same-origin through a reverse proxy.
**Verification:** Recreated the backend container, then confirmed `GET /api/auth/demo-accounts` returns `access-control-allow-origin` for both `http://127.0.0.1:3000` and `http://0.0.0.0:3000`. `uv run ruff check app` passed on host; host `mypy` was blocked by Windows application control, so `docker compose exec -T backend uv run mypy app` passed in the container.
**Reference:** `.env.example`, `.env`, `docker-compose.yml`, `backend/app/config.py`.

### 2026-05-19 — Day 8 — OCR accepts Turkish decimal text

**Context:** Real receipt uploads reached `/api/receipts/upload` but returned `422`, meaning storage/auth/provider setup worked while the model response failed validation.
**Decision/learning:** `LlmReceiptPayload` and item parsing now normalize Turkish amount strings before Decimal validation: examples like `247,50`, `1.247,50 TL`, `247,50 ₺`, and `1.000` are accepted as Decimal values. Tests cover both direct payload validation and a fake vision-model JSON response with Turkish-formatted values. We still do not log raw OCR output because raw OCR/base64 must stay out of logs.
**Why it matters:** Vision models often mirror the receipt locale instead of obeying decimal-dot instructions. Keep OCR parsing tolerant at the boundary, then store money as Decimal/NUMERIC internally.
**Verification:** `uv run pytest tests/test_ocr.py -q`, `uv run ruff check app tests`, `uv run ruff format --check .`, and container `docker compose exec -T backend uv run mypy app` passed. A running-container parser smoke printed `1247.50`, `247.50`, and `0.85` for Turkish-formatted input. Full `uv run pytest -q` currently has one unrelated failure in `tests/test_demo_seed.py::test_demo_seed_creates_parent_logins_child_and_refreshes_insights` from saving-goal baseline setup.
**Reference:** `backend/app/services/ocr.py`, `backend/tests/test_ocr.py`.

### 2026-05-19 — Day 8 — OCR tolerates common model JSON drift

**Context:** One receipt upload succeeded while another returned `422`, showing that provider access worked but some model outputs still failed schema validation.
**Decision/learning:** Receipt OCR parsing now accepts common model drift without logging raw OCR: `total`/`amount`/`grand_total`/`genel_toplam` aliases for `total_amount`, `date`/`datetime`/`tarih` aliases for `occurred_at`, Turkish dates like `12.05.2026 14.32`, and confidence values like `85%`. The fake vision-model test now exercises these drift cases.
**Why it matters:** Receipt images vary heavily; provider output may follow the visual receipt labels rather than our prompt schema. Normalize these safe structural variants before deciding the receipt is unreadable.
**Verification:** `uv run pytest tests/test_ocr.py -q`, `uv run ruff check app tests`, `uv run ruff format --check .`, and container `docker compose exec -T backend uv run mypy app` passed. Rebuilt backend with `docker compose up -d --build backend`; container smoke parsed `total='1.247,50 TL'`, `date='12.05.2026 14.32'`, and `confidence='85%'` into `1247.50`, `2026-05-12T14:32:00+03:00`, and `0.85`.
**Reference:** `backend/app/services/ocr.py`, `backend/tests/test_ocr.py`.

### 2026-05-19 — Day 8 — Receipt item discounts must not reject the receipt

**Context:** Two previously failed receipt uploads were safely replayed from MinIO without printing raw OCR or image contents. Both failed with the same safe validation shape: `items[9].amount` violated `greater_than`, while total/merchant parsing was otherwise usable.
**Decision/learning:** Removed the positive-only constraint from OCR line-item amounts. Receipt totals still require `> 0`, but item rows may be `0,00` or negative because receipts can include bag fees, deposit returns, discounts, promotions, or correction lines. Tests now cover zero and negative item amounts.
**Why it matters:** Do not reject an entire receipt because one line item is a discount/correction. Keep validation strict for the transaction total and tolerant for item details.
**Verification:** `uv run pytest tests/test_ocr.py -q`, `uv run ruff check app tests`, `uv run ruff format --check .`, and container `docker compose exec -T backend uv run mypy app` passed. Safe replay of the two stored failed PNGs now returns `ok amount 503.85 merchant_present True items 11` for both. Rebuilt backend with `docker compose up -d --build backend`; container smoke parsed `0,00` and `-10,00` item amounts.
**Reference:** `backend/app/services/ocr.py`, `backend/tests/test_ocr.py`.

### 2026-05-19 — Day 8 — README screenshots now use committed image filenames

**Context:** Teammate added final screenshot PNGs under `docs/screenshots/` with descriptive Turkish filenames, while `README.md` still referenced the previous numbered placeholder names.
**Decision/learning:** Updated the README screenshot section to point at the actual committed files (`login.png`, `dashboard.png`, `işlemler.png`, `fiş_tarama.png`, `agent_chat.png`, `hedefler.png`, `aile_modu.png`, `dersler.png`) and aligned `docs/screenshots/README.md` with the same names.
**Why it matters:** Future screenshot updates should either keep these filenames stable or update both `README.md` and `docs/screenshots/README.md` together so GitHub renders the public README correctly.
**Reference:** `README.md`, `docs/screenshots/README.md`, `docs/screenshots/*.png`.

### 2026-05-19 — Day 8 — Agent chat screenshot label corrected

**Context:** The `agent_chat.png` screenshot was briefly labelled as both AI chat and voice coach in the README screenshot section.
**Decision/learning:** Corrected the label to `AI Koç Sohbeti` only. Voice capability remains documented in the feature list, but the screenshot itself represents the chat screen and should not be titled as a separate voice-coach screenshot.
**Why it matters:** Screenshot headings must describe the visible screen, not every capability present in the surrounding component.
**Reference:** `README.md`, `docs/screenshots/README.md`, `docs/screenshots/agent_chat.png`.

### 2026-05-19 — Day 8 — README screenshot captions match visible screens

**Context:** Rechecked each committed screenshot against its filename and visible UI after feedback that labels were still too broad.
**Decision/learning:** Updated the README screenshot headings and captions to describe only what each image shows: `Panel`, `İşlemler`, `Fiş Tarama`, `Sohbet`, `Hedefler`, `Aile Modu`, and `Dersler`. The screenshot asset guide now uses the same names and plain descriptions.
**Why it matters:** Public screenshot captions should be literal and reviewable. Do not use a feature name like voice coach, OCR, or family profile unless that specific screenshot visibly shows that flow.
**Reference:** `README.md`, `docs/screenshots/README.md`, `docs/screenshots/*.png`.

### 2026-05-19 — Day 9 — Bug-fixes ported onto current main

**Context:** Created `integration/main-plus-bug-fixes` from current `origin/main` and reviewed `origin/bug-fixes` before combining it.
**Decision/learning:** `origin/bug-fixes` contains one useful auth/logging fix but was based on an older main, so the changes were ported manually instead of raw-merging the branch. Auth now uses direct `bcrypt`, Passlib and `types-passlib` are removed from backend dependencies, SQLAlchemy echo stays off even when debug is enabled, Windows-friendly type-check commands use `uv run python -m mypy app`, and the demo seed fake session handles SQLAlchemy `IN` filters.
**Why it matters:** Future merges from stale branches should inspect the commit delta against current `origin/main` and port only the still-relevant fixes. Do not reintroduce Passlib unless there is a deliberate auth-library decision and verification that bcrypt-version probes no longer log noisy tracebacks.
**Reference:** `backend/app/auth.py`, `backend/app/db.py`, `backend/pyproject.toml`, `backend/uv.lock`, `backend/tests/test_demo_seed.py`, `Makefile`, `README.md`, `SETUP.md`, `docs/manual_test_added_features.md`.

### 2026-05-19 — Day 9 — First-time setup docs must migrate before UI auth

**Context:** Audited `README.md` and `SETUP.md` for a teammate installing the repo from scratch.
**Decision/learning:** Confirmed the backend does not call `metadata.create_all()` or run Alembic on startup, and auth/demo endpoints query `users` directly. First-time setup docs must run `alembic upgrade head` before telling users to register, log in, or load demo-account UI. Also confirmed host-side demo seeding needs `DATABASE_URL=...localhost...` and `MINIO_ENDPOINT=localhost:9000` overrides because `.env.example` defaults to Docker service names.
**Why it matters:** Keep Docker and host setup commands explicit about execution context. If `.env.example` keeps `postgres`/`minio`, host-mode snippets must override them or tell the user to edit `.env` first.
**Reference:** `SETUP.md`, `README.md`, `backend/app/db.py`, `backend/app/routers/auth.py`, `.env.example`.
