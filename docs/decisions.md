...
### 2026-05-19 — Day 9 — First-run smoke removed noisy auth and SQL logs

**Context:** Full first-run verification on Windows found `uv run mypy app` blocked by local Application Control policy, full pytest failing because `FakeSession` did not model SQLAlchemy `IN` filters, Passlib logging a bcrypt compatibility traceback on first auth, and SQLAlchemy echo printing bound auth parameters when `APP_DEBUG=true`.
**Decision/learning:** The fake session matcher now handles `IN` expressions, auth uses `bcrypt` directly instead of Passlib, SQLAlchemy `echo` stays off even in debug mode, and documented/Makefile type-check commands use `uv run python -m mypy app`.
**Why it matters:** New runners should see clean auth logs and should not leak SQL bound parameters such as password hashes. On locked-down Windows hosts, invoking mypy as a Python module avoids executable-policy failures while preserving the same type check.
**Verification:** `uv run ruff check app tests`, `uv run ruff format --check .`, `uv run python -m mypy app`, `uv run pytest -q` (171 passed), `pnpm lint`, `pnpm type-check`, `pnpm format:check`, `pnpm build`, `docker compose up -d --build backend frontend`, `docker compose exec -T backend uv run alembic upgrade head`, `docker compose exec -T backend uv run alembic check`, backend `/health`, frontend `/login`, auth register/login smoke, demo seed, and demo login all passed. The local PowerShell environment does not have `make`, so the underlying Makefile commands were verified directly.
**Reference:** `backend/tests/test_demo_seed.py`, `backend/app/auth.py`, `backend/app/db.py`, `backend/pyproject.toml`, `Makefile`, `SETUP.md`, `docs/manual_test_added_features.md`.
